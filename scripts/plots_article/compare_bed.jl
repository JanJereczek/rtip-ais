include("../intro.jl")
file_t0 = datadir("output/ais/hyster/16km/aqef/pmpt-hydro-25K/1/yelmo2D.nc")
file_t1 = datadir("output/ais/hyster/16km/equil/25K-100/45/0/yelmo2D.nc")
file_t0_hr = datadir("BedMachineAntarctica-v3.nc")

x, y = ncread(file_t0, "xc"), ncread(file_t0, "yc")
nx, ny = length(x), length(y)
nt1 = length(ncread(file_t1, "time"))
z_bed_t0 = ncread(file_t0, "z_bed", start = [1, 1, 1], count = [-1, -1, 1])
z_bed_t1 = ncread(file_t1, "z_bed", start = [1, 1, nt1], count = [-1, -1, 1])
...
fig = Figure(size = (1500, 1120), fontsize = 26)
...
contour!(axs[1, 1], view(mask, i1_ais:i2_ais, j1_ais:j2_ais), levels = [0.01, 1.99], linewidth = 3,
    colormap = cgrad([framecolors[1], framecolors[2]]))
contour!(axs[2, 1], view(mask, i1_ais:i2_ais, j1_ais:j2_ais), levels = [0.01, 1.99], linewidth = 3,
...
Colorbar(fig[nrows+1, :], label = "Bed elevation (km)", width = Relative(0.3), flipaxis = false,
