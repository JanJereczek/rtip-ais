...
xps = [
    ...
    datadir("output/ais/v2/hyster/retreat/aqef/elra"),
    datadir("output/ais/v2/hyster/retreat/aqef/elva"),
...
fig_gia = Figure(size=(1600, 800), fontsize = 24)
ax = Axis(fig_gia[1, 1])
...
axmap = Axis(fig_gia[1, 2], aspect = DataAspect())
cropx, cropy = 20, 35
...
relwidth = 0.35
Colorbar(fig_gia[2, 2], vertical = false, flipaxis = false, width = Relative(relwidth), halign = :left,
    label = L"Bed elevation $z_b$ (km)", ticks = latexifyticks(-6:2:2, 1f3); cmaps["z_bed2"]...)
Colorbar(fig_gia[2, 2], vertical = false, flipaxis = false, width = Relative(relwidth), halign = :right,
...
Legend(fig_gia[2, 2], [elem_1, elem_2], ["Modelled", "Observed"], label = "Grounding zone", valign = 3)

text!(ax, 10.2, 56, color = :black, font = :bold, text="(a)", fontsize = 30)
text!(axmap, -2600, 2150, color = :black, font = :bold, text="(b)", fontsize = 30)
colsize!(fig_gia.layout, 1, 740)
colsize!(fig_gia.layout, 2, 800)
rowsize!(fig_gia.layout, 2, 50)
rowgap!(fig_gia.layout, 1, -70)
colgap!(fig_gia.layout, 1, -30)
fig_gia

save(plotsdir("v2/hysteresis/retreat-gia.png"), fig_gia)
save(plotsdir("v2/hysteresis/retreat-gia.pdf"), fig_gia)