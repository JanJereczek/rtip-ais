...
dfdt_max = 10 ^ ceil(log10(dfdt_max))
...
if region == "recovery"
    ylims!(ax_ramp, (-0.1, 6.8))
    ax_ramp.yticks = 0:2:6
    ax_vol.xticks = 0:20:60
    ax_vol.xminorticksvisible = true
    xlims!(ax_vol, (0, 45))
elseif region == "wais"
    ylims!(ax_ramp, (-0.1, 1.6))
    ax_ramp.yticks = 0:0.5:2
...
for j in eachindex(hr.visc_cases)
    ...
    if length(hr.f[j]) > 1
        heatmap!(axs[j], log10.(hr.dfdt[j] ./ pa), hr.f[j] ./ pa, hr.V[j]; cmap...)
        scatter!(axs[j], log10.(hr.dfdt[j] ./ pa), hr.f[j] ./ pa, markersize = ms1, color = :white)
        if region == "recovery"
            scatter!(axs[j], log10.(hr.dfdt[j][rsl_index] ./ pa), hr.f[j][rsl_index] ./ pa,
                markersize = ms2, color = visc_colors[j], marker = viscmarker)
        end
        hlines!(axs[j], [f_bif ./ pa], color = :gray10, linestyle = :dash, linewidth = 4)
    end
    ...
    if region == "recovery"
        axs[j].yticks = 5.2:0.2:6.8
    elseif region == "wais"
        axs[j].yticks = 0.6:0.2:1.4
    end

    if region == "recovery"
        ylims!(axs[j], (9.3 ./ pa, 11.5 ./ pa))
        xlims!(axs[j], (log10(0.0003 ./ pa)-0.25, log10(0.3 ./ pa)+0.25))
    elseif region == "wais"
        ylims!(axs[j], (1.3 ./ pa, 2.7 ./ pa))
...
for i in eachindex(f_hm_jet)
    scatter!(axs[k], log10.(hr.dfdt[k][idx_jet[i]] ./ pa), hr.f[k][idx_jet[i]] ./ pa,
        markersize = ms2, color = jetmap[i])
    file1D = joinpath(hr.paths[k][idx_jet[i]], "yelmo1D.nc")
    lines!(ax_ramp, ncread(file1D, "time") ./ 1e3, ncread(file1D, "hyst_f_now") ./ pa,
        color = jetmap[i], linewidth = 2)
    lines!(ax_vol, ncread(file1D, "time")[1:100:end] ./ 1e3, ncread(file1D, "V_sle")[1:100:end],
        color = jetmap[i], linewidth = 2)
end
...
for i in eachindex(f_hm)
    ...
    scatter!(axs[k], log10.(hr.dfdt[k][idx[i]] ./ pa), hr.f[k][idx[i]] ./ pa,
...
for i in eachindex(basins)
    scatterlines!(axs[1, 2], f_rtip[i], label = visc_labels[i],
...
for i in eachindex(basins)
    scatterlines!(axs[2, 2], f_rtip[i] .- f_bif, label = visc_labels[i],
        color = visc_colors[i]; slines_opts...)
end
...
axs[1, 2].ylabel = L"$f^\mathrm{crit}$ (K)"
