...
fig = Figure(size = (2100, 2550), fontsize = 40)
nrows, ncols = 3, 2
axs_hm = [Axis(fig[i+1, 1], aspect = DataAspect()) for i in 1:nrows]
axs_tr = [Axis(fig[i+1, 2:3], aspect = AxisAspect(1.6)) for i in 1:nrows]
...
for i in eachindex(dirs)
    ...
    text!(axs_tr[i], x2 + 10, 550, text = "t = $(Int(round(time2D / 1e3, digits=1))) kyr", color = :black)
...
Legend(fig[1, 3], axs_tr[1], nbanks = 2, valign = :bottom, patchsize = (40f0, 20f0))
...
text!(axs_tr[1], x2 + 10, 650, text = L"(b) $-2 \, \sigma$", color = :black, font = :bold)
text!(axs_tr[2], x2 + 10, 650, text = L"(d) $0 \, \sigma$", color = :black, font = :bold)
text!(axs_tr[3], x2 + 10, 650, text = L"(f) $+2 \, \sigma$", color = :black, font = :bold)
