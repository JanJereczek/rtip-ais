...
dirs = [datadir("output/ais/v2/ramps/rsb/$i") for i in 1:9]
for dir in dirs
    aggregate_xp!(hr, dir)
end
...
idx_jet = [argmin( abs.(hr.f[k] .- f_hm_jet[i]) + abs.(rtime_k .- rtime_hm_jet[i]) )
...
jetmap = cgrad(:copper, range(0, stop = 1, length = 10), categorical = true, rev = true)
