...
fig = Figure(size = (1600, 1630), fontsize = 40)
nrows, ncols = 3, 2
axs_tr = [Axis(fig[i+1, 1], aspect = AxisAspect(3)) for i in 1:nrows]
...
for i in eachindex(dirs)
    ...
    band!(axs_tr[i], x[i2:i3], z_bed_vec, z_srf_vec, color = :lightskyblue1)
    lines!(axs_tr[i], x[i2:i3], z_srf_vec, color = :cornflowerblue, linewidth = 4, label = "ice surface")
    ...
    if i < nrows
        ...
        axs_tr[i].xlabel = "Distance along transect (km)"
    end
    text!(axs_tr[i], x2 + 10, 200, text = "t = $(Int(round(time2D / 1e3, digits=1))) kyr", color = :black)

end
# Colorbar(fig[1, 1], label = "Bed elevation (km)", vertical = false, width = Relative(rw), height = Relative(rh), ticks = (-4000:1000:3000, string.(-4:3)); cmaps["z_bed"]...)
# Colorbar(fig[1, 2], label = "Ice surface elevation (km)", vertical = false, width = Relative(rw), height = Relative(rh), ticks = (-4000:1000:3000, string.(-4:3)); cmaps["z_srf"]...)
Legend(fig[1, 1], axs_tr[1], nbanks = 4, valign = :bottom, patchsize = (40f0, 20f0))
...
text!(axs_tr[1], x2 + 10, 600, text = "a", color = :black, font = :bold)
text!(axs_tr[2], x2 + 10, 600, text = "b", color = :black, font = :bold)
text!(axs_tr[3], x2 + 10, 600, text = "c", color = :black, font = :bold)
text!(axs_tr[1], x2 + 10, 400, text = L"$\Delta \eta = -2 \, \sigma$", color = :black, font = :bold)
text!(axs_tr[2], x2 + 10, 400, text = L"$\Delta \eta = 0 \, \sigma$", color = :black, font = :bold)
text!(axs_tr[3], x2 + 10, 400, text = L"$\Delta \eta = +2 \, \sigma$", color = :black, font = :bold)

colsize!(fig.layout, 1, 1350)
rowsize!(fig.layout, 1, 60)
rowgap!(fig.layout, 0)

save(plotsdir("v2/rtip/transect-rsb.png"), fig)