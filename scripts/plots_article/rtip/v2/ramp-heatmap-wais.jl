...
hr = HeatmapRtipRate{Float32}(visc_cases)
...
nx, ny = size(X)
i1, i2, i3, i4 = [60, 90, 110, 160]
j1, j2, j3, j4 = [105, 150, 170, 205]
...
f_bif = fill(1.95, hr.n_visc_cases)
crange = (53, 56)
trange = (1, 7)
vrange = (57, 57.8)
zrange = (-870, -864)
...
fig_wais = Figure(size = (1100, 750), fontsize = fs)
ha = fig_wais[1, 1:3] = GridLayout()
ra = fig_wais[2, 1] = GridLayout()
ba = fig_wais[2, 2:3] = GridLayout()
...
xlims!(ax_bed, trange)
ylims!(ax_bed, zrange)
ax_bed.yticks = zrange[1]:2:zrange[2]
...
xlims!(ax_vol, trange)
ylims!(ax_vol, vrange)
ax_vol.yticks = vrange[1]:0.2:vrange[2]-0.1
...
for j in eachindex(hr.visc_cases)
    ...
    if length(hr.f[j]) > 1
        hdfdt_idx = get_dfdt_idx(hr.dfdt[j])
        heatmap!(axs[j], hdfdt_idx, hr.f[j] ./ pa .+ f_pd, hr.V[j]; cmap...)
        scatter!(axs[j], hdfdt_idx, hr.f[j] ./ pa .+ f_pd, markersize = ms1,
            color = :white)
        scatter!(axs[j], hdfdt_idx[rsl_index], hr.f[j][rsl_index] ./ pa .+ f_pd,
    ...
    # axs[j].xticks = (1:10, string.(sort(unique(hr.dfdt[j]))))
    axs[j].xticklabelrotation = π / 2
    ylims!(axs[j], extrema(hr.f[1]) ./ pa .+ f_pd .+ (-0.05, 0.05))
    axs[j].xticks = (1:2:10, ["2e-3", "5.4e-3", "2e-2", "5.4e-2", "2e-1",
        "5.4e-1", "2e0", "5.4e0", "2e1", "5.4e1"][1:2:end])
    vlines!(axs[j], [6.3, 8.2], color = :gray50, linewidth = 3, linestyle = :dash)
...
axislegend(ax_vol, position = :rb, nbanks = 3, labelsize = 14)
...
text!(ax_bed, trange[1] + 0.2, zrange[2] - 1, text = "f", color = :black, font = :bold)
text!(ax_vol, trange[1] + 0.2, vrange[1] + 0.08, text = "g", color = :black, font = :bold)
text!(ax_hm[1], x1 + 80, y1 + 60, text = "h", color = :white, font = :bold)
text!(ax_hm[2], x1 + 80, y1 + 60, text = "i", color = :white, font = :bold)

Colorbar(fig_wais[3, 2], label = L"Final AIS volume $\mathrm{(m \, SLE)}$", vertical = false,
    width = Relative(rw_cbar), flipaxis = false, halign = :left; cmap...)
Colorbar(fig_wais[3, 2:3], label = "Ice surface elevation (km)", vertical = false,
    width = Relative(rw_cbar/2), flipaxis = false, ticks = (vcat(1, 1e3:1e3:4e3),
    string.(0:4)); cmaps["z_srf"]...)
Colorbar(fig_wais[3, 3], label = "Bed elevation (km)", vertical = false,
    width = Relative(rw_cbar), flipaxis = false, ticks = latexifyticks(-6:2:4, 1e3),
    halign = :right; cmaps["z_bed"]...)

colsize!(ra, 1, 280)
rowsize!(fig_wais.layout, 2, 350)
rowgap!(fig_wais.layout, 10)
rowgap!(ra, 5)
rowgap!(fig_wais.layout, 2, -35)
colgap!(ha, 5)
fig_wais

save(plotsdir("v2/rtip/ramp-heatmap-wais.png"), fig_wais)
save(plotsdir("v2/rtip/ramp-heatmap-wais.pdf"), fig_wais)