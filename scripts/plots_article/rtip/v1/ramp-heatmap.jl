...
if region == "recovery"
    dirs = [
        ...
        datadir("output/ais/ramps/16km/ramps-recovery-sigmarange-120kyr"),
        datadir("output/ais/ramps/16km/ramps-recovery-sigmarange-385kyr"),
    ]
elseif region == "wais"
    dirs = [
...
for j in eachindex(hr.visc_cases)
    ...
    if length(hr.f[j]) > 1
        ...
        scatter!(axs[j], log10.(hr.dfdt[j] ./ pa), hr.f[j] ./ pa .+ f_pd, markersize = ms1,
            color = :white)
        if (region == "recovery") && plot_highlight_scatter
    ...
    if region == "recovery"
        ...
        xlims!(axs[j], (log10(0.00003 ./ pa)-0.25, log10(0.3 ./ pa)+0.25))
    elseif region == "wais"
...
save(plotsdir("16km/rtip/ramp-heatmap-$region-ext-$(plot_highlight_scatter).png"), fig)
save(plotsdir("16km/rtip/ramp-heatmap-$region-ext-$(plot_highlight_scatter).pdf"), fig)
